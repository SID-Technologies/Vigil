...
func TestDetector_OpensAfterThreeFailures(t *testing.T) {
	...
	assertOutageCount(t, ctx, client, "target:router_icmp", 0)
	...
	assertOutageCount(t, ctx, client, "target:router_icmp", 1)

	// Verify the persisted outage shape.
	row := mustGetOnly(t, ctx, client, "target:router_icmp")
...
func TestDetector_UpdatesWhileOngoing(t *testing.T) {
	...
	row := mustGetOnly(t, ctx, client, "target:google_dns_icmp")
...
func TestDetector_ClosesOnSuccess(t *testing.T) {
	...
	assertOutageCount(t, ctx, client, "target:teams_tcp443", 1)
	...
	row := mustGetOnly(t, ctx, client, "target:teams_tcp443")
	if row.EndTsUnixMs == nil {
		t.Fatalf("end_ts not set after close")
	}
...
func TestDetector_DoesNotReopenAfterClose(t *testing.T) {
	...
	assertOutageCount(t, ctx, client, "target:x", 1)
	...
	assertOutageCount(t, ctx, client, "target:x", 2)
}
...
func TestDetector_NetworkScopeWhenAllFail(t *testing.T) {
	...
	assertOutageCount(t, ctx, client, "network", 1)
}
...
func TestDetector_NetworkScopeNotTriggeredIfOneSucceeds(t *testing.T) {
	...
	assertOutageCount(t, ctx, client, "target:a", 1)
	assertOutageCount(t, ctx, client, "network", 0)
}
...
func assertOutageCount(t *testing.T, ctx context.Context, client *ent.Client, scope string, want int) {
	t.Helper()
...
func mustGetOnly(t *testing.T, ctx context.Context, client *ent.Client, scope string) *ent.Outage {
	t.Helper()
